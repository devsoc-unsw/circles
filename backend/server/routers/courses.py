...
def getLegacyCourses(year, term):
    ...
    result = {
        c["code"]: c["title"] for c in archivesDB[year].find() if term in c["terms"]
    }
...
def unselectCourse(userData: UserData, lockedCourses: list, unselectedCourse: str):
    ...
    """
    affectedCourses = User(fixUserData(userData.dict())).unselect_course(
        unselectedCourse, lockedCourses
    )
