...
def cache_program_mappings():
    ...
    for program in programs.values():
        ...
        if re.match(r"business", program["title"], flags=re.IGNORECASE):
            mappings["BUSN#"][program["code"]] = 1
            mappings["ZBUS#"][program["code"]] = 1
        if re.match(r"commerce", program["title"], flags=re.IGNORECASE):
            mappings["COMM#"][program["code"]] = 1
            mappings["ZBUS#"][program["code"]] = 1
        if re.match(r"economics", program["title"], flags=re.IGNORECASE):
            mappings["ECON#"][program["code"]] = 1
            mappings["ZBUS#"][program["code"]] = 1
        if re.match(r"information systems", program["title"], flags=re.IGNORECASE):
            mappings["INFS#"][program["code"]] = 1
            mappings["ZBUS#"][program["code"]] = 1
        if re.match(r"data science and decisions", program["title"], flags=re.IGNORECASE):
            mappings["DATA#"][program["code"]] = 1
        if re.match(r"computer science", program["title"], flags=re.IGNORECASE):
            mappings["COMP#"][program["code"]] = 1
        if re.match(r"advanced maths", program["title"], flags=re.IGNORECASE):
            mappings["MATH#"][program["code"]] = 1
        if re.match(r"advanced science", program["title"], flags=re.IGNORECASE):
            mappings["ASCI#"][program["code"]] = 1
        if re.match(r"education", program["title"], flags=re.IGNORECASE):
            mappings["EDST#"][program["code"]] = 1
        if re.match(r"law", program["title"], flags=re.IGNORECASE):
            mappings["LAWS#"][program["code"]] = 1
        if re.match(r"criminology", program["title"], flags=re.IGNORECASE):
            mappings["CRIM#"][program["code"]] = 1
        if re.match(r"international studies", program["title"], flags=re.IGNORECASE):
            mappings["INST#"][program["code"]] = 1
        if re.match(r"social science", program["title"], flags=re.IGNORECASE):
            mappings["SOSS#"][program["code"]] = 1
        if re.match(r"social work", program["title"], flags=re.IGNORECASE):
            mappings["SOCW#"][program["code"]] = 1
        if re.match(r"music", program["title"], flags=re.IGNORECASE):
            mappings["MUSC#"][program["code"]] = 1
        if re.match(r"design", program["title"], flags=re.IGNORECASE):
            mappings["DDES#"][program["code"]] = 1
        if re.match(r"political", program["title"], flags=re.IGNORECASE):
            mappings["PPEC#"][program["title"]] = 1

        # ARTS != MEDIA ARTS != MEDIA
        if re.match(r"media arts", program["title"], flags=re.IGNORECASE):
            mappings["DART#"][program["code"]] = 1
        elif re.match(r"arts", program["title"], flags=re.IGNORECASE):
            mappings["ARTS#"][program["code"]] = 1
        elif re.match(r"media", program["title"], flags=re.IGNORECASE):
