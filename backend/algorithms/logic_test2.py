...
def test_level_condition_simple():
    ...
    # WAM conditions
    l1_70wam_cond = create_condition(["(", "70WAM", "in", "L1", ")"])
    l1_90wam_cond = create_condition(["(", "90WAM", "in", "L1", ")"])
    l2_70wam_cond = create_condition(["(", "70WAM", "in", "L2", ")"])
    l2_90wam_cond = create_condition(["(", "90WAM", "in", "L2", ")"])
    l3_90wam_cond = create_condition(["(", "90WAM", "in", "L3", ")"])


    # TODO: None grades should pass (with warning) we just return True for now
    assert (l1_70wam_cond.is_unlocked(user))["result"] == True
    assert (l1_90wam_cond.is_unlocked(user))["result"] == True
    assert (l2_70wam_cond.is_unlocked(user))["result"] == True
    assert (l2_90wam_cond.is_unlocked(user))["result"] == True
    assert (l3_90wam_cond.is_unlocked(user))["result"] == True
    ...
    assert (l1_70wam_cond.is_unlocked(user1))["result"] == True
    assert (l1_90wam_cond.is_unlocked(user1))["result"] == False
    assert (l2_70wam_cond.is_unlocked(user1))["result"] == True
    assert (l2_90wam_cond.is_unlocked(user1))["result"] == True
    assert (l3_90wam_cond.is_unlocked(user1))["result"] == False
...
def test_level_course_condition():
    ...
    # WAM conditions
    l1_comp_70wam_cond = create_condition(["(", "70WAM", "in", "L1", "COMP", ")"])
    l1_comp_90wam_cond = create_condition(["(", "90WAM", "in", "L1", "COMP", ")"])
    l2_comp_70wam_cond = create_condition(["(", "70WAM", "in", "L2", "COMP", ")"])
    l2_comp_90wam_cond = create_condition(["(", "90WAM", "in", "L2", "COMP", ")"])
    l1_math_70wam_cond = create_condition(["(", "70WAM", "in", "L1", "MATH", ")"])
    l1_math_90wam_cond = create_condition(["(", "90WAM", "in", "L1", "MATH", ")"])
    l2_math_70wam_cond = create_condition(["(", "70WAM", "in", "L2", "MATH", ")"])
    l2_math_90wam_cond = create_condition(["(", "90WAM", "in", "L2", "MATH", ")"])

    # TODO: None grades should pass (with warning) we just return True for now
    assert (l1_comp_70wam_cond.is_unlocked(user))["result"] == True
    assert (l1_comp_90wam_cond.is_unlocked(user))["result"] == True
    assert (l2_comp_70wam_cond.is_unlocked(user))["result"] == True
    assert (l2_comp_90wam_cond.is_unlocked(user))["result"] == True
    assert (l1_math_70wam_cond.is_unlocked(user))["result"] == True
    assert (l1_math_90wam_cond.is_unlocked(user))["result"] == True
    assert (l2_math_70wam_cond.is_unlocked(user))["result"] == True
    assert (l2_math_90wam_cond.is_unlocked(user))["result"] == True
    ...
    assert (l1_comp_70wam_cond.is_unlocked(user1))["result"] == True
    assert (l1_comp_90wam_cond.is_unlocked(user1))["result"] == False

    # TODO: How to deal with this? None grade but we haven't even taken the courses...
    assert (l2_comp_70wam_cond.is_unlocked(user1))["result"] == True
    assert (l2_comp_90wam_cond.is_unlocked(user1))["result"] == True
    assert (l1_math_70wam_cond.is_unlocked(user1))["result"] == True
    assert (l1_math_90wam_cond.is_unlocked(user1))["result"] == True
    assert (l2_math_70wam_cond.is_unlocked(user1))["result"] == True
    assert (l2_math_90wam_cond.is_unlocked(user1))["result"] == False
